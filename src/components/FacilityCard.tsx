      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex items-start justify-between mb-3 sm:mb-4">
          <div className="flex-1">
            <h3 className="text-lg sm:text-xl font-bold text-white mb-2 line-clamp-2 group-hover:text-[#B79D9B] transition-colors">
              {facility.name}
            </h3>
            <div className="flex items-center space-x-2 text-[#B0A6A7] mb-2">
              ...
              <button
                ...
                className="text-xs sm:text-sm line-clamp-1 hover:text-[#B79D9B] transition-colors cursor-pointer underline decoration-dotted flex items-center space-x-1"
          ...
          {/* Rating */}
          {facility.rating && (
            <div className="flex items-center space-x-1 bg-[#72676F]/20 px-2 py-1 rounded-lg">
              <Star className="w-4 h-4 text-[#B79D9B] fill-current" />
              <span className="text-xs sm:text-sm font-medium text-white">{facility.rating}</span>
              {facility.user_ratings_total && (
                <span className="text-xs text-[#B0A6A7]">({facility.user_ratings_total})</span>
              )}
            </div>
          )}
        </div>
        ...
        {specialtyTags.length > 0 && (
          <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
            {specialtyTags.map((tag, index) => (
              <span
                key={index}
                className="px-2 py-1 text-xs font-medium bg-[#9A8D96]/20 text-[#B0A6A7] rounded-md border border-[#9A8D96]/30"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="space-y-2 mb-4">
          {facility.phone && (
            <div className="flex items-center space-x-2 text-[#B0A6A7]">
        ...
        {facility.opening_hours && (
          <div className="mb-4">
            <div className="flex items-center space-x-2 mb-2">
              <Clock className="w-4 h-4 text-[#B0A6A7]" />
              <span className="text-sm font-medium text-[#B0A6A7]">Opening Hours</span>
            ...
            {facility.opening_hours.weekday_text && (
              <div className="text-xs text-[#B0A6A7] space-y-1">
        ...
        {/* Expandable Content */}
        {isExpanded && (
          <div className="border-t border-[#B0A6A7]/20 pt-4 mt-4">
            {/* Reviews */}
            {facility.reviews && facility.reviews.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-semibold text-white mb-2 flex items-center">
                  <Star className="w-4 h-4 mr-1 text-[#B79D9B]" />
                  Reviews
                </h4>
                <div className="space-y-3 max-h-32 overflow-y-auto">
                  {facility.reviews.map((review, index) => (
                    <div key={index} className="bg-[#72676F]/10 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-white">{review.author_name}</span>
                        <div className="flex items-center space-x-1">
                          <Star className="w-3 h-3 text-[#B79D9B] fill-current" />
                          <span className="text-xs text-[#B0A6A7]">{review.rating}</span>
                        </div>
                      </div>
                      <p className="text-xs text-[#B0A6A7] line-clamp-2">{review.text}</p>
                      <span className="text-xs text-[#72676F]">{review.relative_time_description}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              {facility.vicinity && (
                <div>
                  <span className="text-[#B0A6A7]">Area:</span>
              ...
              {facility.geometry?.location && (
                <div>
                  <span className="text-[#B0A6A7]">Coordinates:</span>
        ...
        {/* Action Button */}
        <div className="mt-4 pt-4 border-t border-[#B0A6A7]/20">
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div className="mt-3 text-center">
          <div className="inline-flex items-center space-x-1 text-[#B0A6A7] text-xs">
